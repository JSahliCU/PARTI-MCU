...
with open('HF_grid_search.csv', 'w') as f:
    print('Amplifier Voltage, Solid Capacitance, Split Capacitance, \
            Immediate Reading Solid, Immediate Reading Split, Immediate Reading Combined', file=f)
    ...
    for solid_cap in range(nominal_capacitance_solid - search_range, nominal_capacitance_solid + search_range + 1):
        for split_cap in range(nominal_capacitance_split - search_range, nominal_capacitance_split + search_range + 1):
            ...
            print('Solid voltage read {0}, with cap = {1}'.format(
                immediate_reading_solid, sm.tuner.tb_solid.current_capacitance_pF))
            print('Split voltage read {0}, with cap = {1}'.format(
                immediate_reading_split, sm.tuner.tb_split.current_capacitance_pF))

            print('5, {0}, {1}, \
                {2}, {3}, {4}'.format(
                    solid_cap, split_cap,
                    immediate_reading_solid, immediate_reading_split, 
                    immediate_reading_solid + immediate_reading_split
                ), file=f)
